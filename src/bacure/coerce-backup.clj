...
(def ^:dynamic *verbose*
  ...
  discarded. Mostly for development purposes." true)
...
(defn c-date-time [string]
  (let [t (clj-time.local/to-local-date-time
    ...
    (DateTime. (Date. gregorian-calendar)
               (Time. gregorian-calendar)))) ;we don't need to add any time
...
;; we lose precision here (hundredth vs millis). Be careful with the tests.
...
(defmethod bacnet->clojure com.serotonin.bacnet4j.type.primitive.Time
  ...
  (.toString o)) ;; careful, returns hundredth instead of millis
            ...
            (let [object-identifier (.getObjectIdentifier p-ref)
              ...
              (into {}
                    [[:object-identifier (bacnet->clojure object-identifier)]
                     [(object-to-keyword (.getPropertyIdentifier p-ref)) value]])))]
